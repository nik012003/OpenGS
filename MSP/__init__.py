...
class MSP(object):
    ...
    def get_analog(self):
        ...
        values = dict()
        values['battery_voltage'] = int.from_bytes(payload[0:1], byteorder='little', signed=False)
        values['mah_drawn'] = int.from_bytes(payload[1:3], byteorder='little', signed=False)
        values['rssi'] = int.from_bytes(payload[3:5], byteorder='little', signed=False)
        values['amp'] = int.from_bytes(payload[5:7], byteorder='little', signed=True)
        return values
